<template>
  <div class="scheduled-tasks-page">
    ...
    <div class="save-button-container">
      <el-button type="primary" @click="handleSave" :loading="isSaving">保存所有定时任务设置</el-button>
    </div>
...
function updateStateFromConfig() {
  ...
  savedTasks.forEach(savedTask => {
    if (localTaskStates[savedTask.id]) {
      ...
      // 新增：初始化时就解析一次已有的CRON表达式
      parseCron(localTaskStates[savedTask.id]);
    }
  });

  definedTasks.value.forEach(taskDef => {
    if (!localTaskStates[taskDef.id]) {
      localTaskStates[taskDef.id] = {
        id: taskDef.id,
        name: taskDef.name,
        enabled: false,
...
const handleSave = async () => {
  ...
  if (result1.success && result2.success) {
    ElMessage.success('所有定时任务相关设置已成功保存！');
  } else {
    ElMessage.error(`保存失败: ${result1.message || result2.message}`);
  }
